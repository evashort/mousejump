...
namespace MouseJumpSettings
{
    public class Settings : IDisposable
    {
        ...
        internal Views.LabelList GetChildAtIndex(string parent, int index)
        {
            JsonObject definition = Definitions.GetNamedObject(parent);
            string operation = definition.GetNamedString("operation");
            if (operation == "join")
            {
                string name = definition.GetNamedArray("input").GetStringAt((uint)index);
                return new Views.LabelList(this, name, parent, index);
            }
            else if (operation == "merge")
            {
                JsonValue inputValue = definition.GetNamedValue("input");
                if (inputValue.ValueType == JsonValueType.Array)
                {
                    foreach (IJsonValue groupValue in inputValue.GetArray())
                    {
                        if (groupValue.ValueType == JsonValueType.Object)
                        {
                            JsonObject group = groupValue.GetObject();
                            ...
                            {
                                string name = group.OrderByDescending(x => x.Value.GetNumber()).ThenBy(x => x.Key).Take(index).First().Key;
                                return new Views.LabelList(this, name, parent, index);
                            }
                        ...
                        {
                            return new Views.LabelList(this, groupValue.GetString(), parent, index);
                        }
                        else
                        {
                            index--;
                ...
                {
                    string name = inputValue.GetObject().OrderByDescending(x => x.Value.GetNumber()).ThenBy(x => x.Key).Take(index).First().Key;
                    return new Views.LabelList(this, name, parent, index);
                }
            }

            return new Views.LabelList(this, definition.GetNamedString("input"), parent);
        }

        internal int CountChildren(string parent)
        {
            JsonObject definition = Definitions.GetNamedObject(parent);
            string operation = definition.GetNamedString("operation");
            if (operation == "join")
            {
                return definition.GetNamedArray("input").Count;
            }
            else if (operation == "merge")
            {
                JsonValue inputValue = definition.GetNamedValue("input");
                ...
                {
                    int total = 0;
                    foreach (IJsonValue group in inputValue.GetArray())
                    {
                        total += group.ValueType == JsonValueType.Object
                            ? group.GetObject().Count : 1;
                    }
                ...
                {
                    return inputValue.GetObject().Count;
                }
            }

            return operation == "edit" ? 1 : 0;
        }

        internal object EnumerateChildren(string parent)
        {
            return new ChildEnumerator(
                this,
                parent,
                Definitions.GetNamedObject(parent).GetNamedValue("input"));
        }
